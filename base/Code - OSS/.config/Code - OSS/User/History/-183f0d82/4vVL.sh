...
stow_from_config() {
  ...
  local config_file="$2"
  local mode="$3"  # Optional: pass --unstow to unstow
  ...
  cd "$base_path" || { echo "Error: Failed to cd into $base_path"; return 1; }
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ -d "$line" ]]; then
      local target_dir="$HOME/.config/$line"
      local backup_dir="$HOME/.config/${line}_backup_$(date +%Y%m%d%H%M%S)"

      if [[ "$mode" == "--unstow" ]]; then
        if [[ -d "$target_dir" ]]; then
          read -rp "Backup and remove existing config '$target_dir'? (y/n): " ans
          if [[ "$ans" =~ ^[Yy]$ ]]; then
            echo "Backing up $target_dir to $backup_dir"
            mv "$target_dir" "$backup_dir"
            echo "Unstowing $line from $HOME"
            stow -D -t "$HOME" "$line"
            echo "Restoring backup $backup_dir to $target_dir"
            mv "$backup_dir" "$target_dir"
            echo "Backup restored, removing backup directory"
            # No backup directory left after mv, so nothing to remove
          else
            echo "Skipping unstow of $line"
          fi
        else
          echo "Unstowing $line"
          stow -D -t "$HOME" "$line"
        ...
        # Normal stow
        if [[ -d "$target_dir" ]]; then
          read -rp "Directory $target_dir already exists. Remove before stowing? (y/n): " ans
          if [[ "$ans" =~ ^[Yy]$ ]]; then
            echo "Backing up $target_dir to $backup_dir"
            mv "$target_dir" "$backup_dir"
            echo "Stowing $line to $HOME"
            stow -t "$HOME" "$line"
          else
            echo "Skipping stow of $line"
          fi
        else
          echo "Stowing $line to $HOME"
          stow -t "$HOME" "$line"
