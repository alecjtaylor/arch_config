...
stow_from_config() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ -d "$line" ]]; then
      ...
      # If the config target already exists, prompt to remove
      if [[ -e "$target_path" && "$mode" != "--unstow" ]]; then
        echo "⚠️  Existing config detected at: $target_path"
        read -p "❓ Remove existing '$target_path' before stowing '$line'? (y/n): " yn
        case "$yn" in
          [Yy]* ) rm -rf "$target_path" && echo "🗑️  Removed $target_path" ;;
          [Nn]* ) echo "⏩ Skipping $line"; continue ;;
          * ) echo "❌ Invalid response. Skipping $line."; continue ;;
        esac
      fi

      echo "→ Running: stow ${stow_args[*]} $line"
      stow "${stow_args[@]}" "$line"
    else
